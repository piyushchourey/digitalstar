...
class  Product_Model Extends CI_Model {
	...
	public function getAllprdct($tableName,$wherefield="",$id="")
	{
	...
	public function getAllprdctBycategory($wherefield="",$id="")
	{
		$this->db->select('products.*,category.name as category_name,category.category_type as cat_type,category_type_info.type_name as category_type_name,category_type_info.pack_type as category_pack_type,category_type_info.pack_timing as category_pack_timing');
		$this->db->from('products');
		$this->db->join("category","category.id=products.category_id");
		$this->db->join("category_type_info","category_type_info.id=category.category_type");
	...
	public function getAllprdctBycategory1($wherefield="",$id="")
	{
		...
		if(!empty($product_arr))
		{
			...
			{ 
				if($prd['category_id'])
				{
					$this->db-> where('id', $prd['category_id']);
					$category_arr = $this->db->get('category')->row_array();
					$prd['category'] = $category_arr;
					if(!empty($category_arr))
					{
						if($category_arr['category_type'])
						{
							$this->db-> where('id', $category_arr['category_type']);
							$category_type_arr = $this->db->get('category_type_info')->row_array();
							$prd['category']['type'] = $category_type_arr;
						}
	...
	public function getAllprdctBycategory_demo($pack)
	{
		foreach ($pack as $p) { 
		$this->db->select('products.*,category.name as category_name,category.category_type as cat_type,category_type_info.type_name as category_type_name,category_type_info.pack_type as category_pack_type,category_type_info.pack_timing as category_pack_timing');
		$this->db->from('products');
		...
		$this->db->join("category_type_info","category_type_info.id=category.category_type");
		$this->db-> where('products.status', 1);
		$this->db-> where('products.pack_type', $p);
		
		$product_arr[$p] = $this->db->get()->result_array();
		}
